<?php
    ...
    {    
        ...
        $file_link_name = "post_info_2_".$PDRequestID."_".$file_index."_".$file_name;

        $result = move_uploaded_file($_FILES["files"]["tmp_name"][0], $output_dir.$file_link_name);
        $PAReqInfo2ID = insertAttachToDB($dbConn, $PDRequestID, $file_link_name, $file_name);

        echo json_encode($PAReqInfo2ID); 
    }
    
    function insertAttachToDB($dbConn, $PDRequestID, $FileLinkName, $FileName) {        
        $ResultID = "";
        $query = "INSERT INTO [IVCPD].[dbo].[PAReqInfo2Attach] "
